...
<section class="admin_state_list">
        ...
        <div class="card">
            <div class="col-md-12">
                ...
                <div class="card-body col-md-5 float-right">
                    <form action="/admin/state/{{ $country->id }}" method="POST">
                        @csrf
                ...
                <div class="table-responsive">
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach ($states as $state)
                                <tr>
                                    <td>
                                        <a href="/admin/state/show/{{$state->id}}" title="View country {{$state->name}}">
                                            {{ $state->name }}
                                        </a>
                                    </td>
                                    <td class="d-flex justify-content-sm-center">
                                        <a href="/admin/state/edit_state/{{$state->id}}" title="Edit state {{$state->name}}" class="mr-2"><i class="text-success" data-feather="edit"></i></a>
                                        <form action="/admin/state/delete/{{$state->id}}" method="POST">
                                            @method('DELETE')
                                            @csrf
