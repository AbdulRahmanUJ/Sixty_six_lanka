...
@section('title', 'Admin | Show Sender')
@section('content')
@section('dashboard_title', 'Details of Sender '.$sender->name)
<section class="admin_show_sender">
        ...
        <div class="row flex justify-content-center">
                        ...
                        @foreach ($sender_address as $item)
                            ...
                            <div class="row float-right">
                                @if (count($sender_address)>1)
                                    <form action="/admin/sender_address/delete/{{ $item->id }}" method="post">
                                        @method('DELETE')
                                        @csrf
                                        <button title="Delete {{ $sender->name }}s Address" type="submit" onclick="alert('Are you sure you want to delete this Address')" id='deleteAddress'  class='btn text-danger deleteAddress'><i data-feather="trash-2"></i></button>
                                    </form>
                                @endif
                                <a href="/admin/sender_address/edit/{{$item->id}}" id='editAddress' class='btn text-primary editAddress'><i data-feather="edit"></i></a>
                            </div>
                            <br>
                            <hr style="border-color: red">
                        @endforeach
            ...
            <div class="row col-12 flex justify-content-center">
                <a href="/admin/sender_address/add/{{$sender->id}}" class="border border-info rounded bg-info text-light py-1 px-3 ">Add New Address</a>&nbsp;&nbsp;&nbsp;
                <a href="/admin/sender/edit/{{$sender->id}}" class="border border-secondary rounded bg-secondary text-light py-1 px-3">Edit</a>&nbsp;&nbsp;&nbsp;
                <form action="/admin/sender/delete/{{$sender->id}}" method="POST">
                    @method('DELETE')
                    @csrf
...
{{-- <script type="text/javascript">
    $(document).ready(function () {
        ...
        // var url = e.target;
            $.ajax(
                {
                url: "/admin/sender_address/delete/"+id, //or you can use url: "movie/"+id,
                type: 'DELETE',
                data: {
                    _token: token,
                        id: id
