...
@section('title', 'Admin | Senders')
@section('content')
@section('dashboard_title', 'Edit Sender Address')
<section class="edit_sender_address">
    <div class="card">
        <div class="card-body">
            <form action="/admin/sender_address/{{ $sender_address->id }}" name="sender_form" method="POST">
                @method('PATCH')
                @csrf
                <div class="form-row">
                    <div class="form-group col-md-12">
                        <label for="address">Address</label>
                        <input type="text" name="address" value="{{old('address')}} {{ $sender_address->address }}" class="form-control" id="inputAddress" placeholder="Address" required>
                        <span style="color:red">{{$errors->first('address')}}</span>
                    </div>
                    <div class="form-group col-md-4">
                        <label for="country">Country</label>
                        <select name="country" id="country" value="{{old('country')}}" class="form-control" required>
                            <option value="{{ $sender_address->country_id }}">{{ $sender_address->country_name }}</option>
                    ...
                    <div class="form-group col-md-4 state hide">
                        ...
                        <select class="form-control" name="state" value="{{old('state')}}" id="state" required>
                            <option value="{{ $sender_address->state_id }}">{{ $sender_address->state_name }}</option>
                            </select>
                    ...
                    <div class="form-group col-md-2 city hide">
                        ...
                        <select class="form-control" name="city" value="{{old('city')}}" id="city" required>
                            <option value="{{ $sender_address->city_id }}">{{ $sender_address->city_name }}</option>
                            </select>
                        <span style="color:red">{{$errors->first('city')}}</span>
                    </div>
                    <div class="form-group col-md-2">
                        <label for="inputZip">Zip Number</label>
                        <input type="text" name="zip" value="{{old('zip')}} {{ $sender_address->zip }}" class="form-control" id="inputZip" placeholder="150600" required>
