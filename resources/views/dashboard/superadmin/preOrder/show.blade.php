...
@section('title', 'Super Admin | Show pre_order_package')
<style>
    #more{
        /* display: none; */
    }
...
<section class="pre_order_package_show">
    ...
    <div class="row">
        <div class="col-md-4">
            @if ($pre_order_package->pre_order_user)
                <div class="card mb-3">
                    <h3 class="text-center pt-3">Pre Order User</h3>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-5">
                            ...
                            <div class="col-sm-7 text-secondary">
                                @foreach ($pre_order_user as $pre_order_user)
                                    <a href="/superadmin/pre_order_user/show/{{$pre_order_user->id}}">{{$pre_order_user->name}}</a>
                                @endforeach
                            </div>
                        </div>
                        <hr>
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-sm-7 text-secondary">
                            ...
                            <a href="/superadmin/sender/show/{{ $pre_order_package->sender->id }}">{{$pre_order_package->sender->name}}</a>
                            @else
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-7 text-secondary">
                                @if ($pre_order_package->status==0)
                                    <span class="bg-danger py-2 px-3 text-light">No</span>
                                @else
                                <span class="bg-Success py-2 px-3 text-light">Yes</span>
                                @endif
                            </div>
                        </div>
                        <hr>
                            ...
                            @if (count($pre_orders)>0)
                                @foreach ($pre_orders as $pre_order)
                                    <tr>
                                        ...
                                        <td>{{ $pre_order->weight }}</td>
                                        <td>{{ $pre_order->length }}</td>
                                        <td>{{ $pre_order->width }}</td>
                                        <td>{{ $pre_order->height }}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
    ...
    <div class="container d-flex justify-content-left">
        <a href="/superadmin/pre_order_package/edit/{{$pre_order_package->id}}" class="border-white bg-white rounded text-primary text-center py-1 px-2"><i data-feather="edit" aria-hidden="true"></i> Edit</a> &nbsp; &nbsp;
        <a href="/superadmin/pre_order_package/preview/{{$pre_order_package->id}}" class="border-white bg-white rounded text-success text-center py-1 px-2"><i data-feather="file-text" aria-hidden="true"></i> Preview</a> &nbsp; &nbsp;
        <form action="/superadmin/pre_order_package/delete/{{$pre_order_package->id}}" method="POST">
            @method('DELETE')
            @csrf
